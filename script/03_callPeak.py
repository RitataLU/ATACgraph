...
def main():
	...
	if (args.separate == 1):
		...
		subprocess.call('''macs2 callpeak -t %s --nomodel --shift -%s --extsize %s -n %s 2>&1'''%(input_bam,shiftSize,extSize,Outname), shell=True)
		#Making bigwig/ bamcoverage limit: cannot use minus
                ...
                #-f: paired end
                subprocess.call('''macs2 callpeak -t %s --format BAMPE -n %s 2>&1'''%(input_bam,Outname), shell=True)
                #Making bam coverage
